...
export default function Writings(){const[filter,setFilter]=useState("All");const shown=filter==="All"?posts:posts.filter(p=>p[2]===filter);return <main><SiteHeader/><section className="writings-hero"><div/><div className="inner-shade"/><section><p className="kicker">Notes from the field</p><h1>Writings & Ideas</h1><p>Thoughts on business, power,<br/>strategy, and human behavior.</p></section></section><section className="articles"><div className="filters light" role="group" aria-label="Filter articles">{filters.map(f=><button className={filter===f?"active":""} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="article-list">{shown.map(([date,title,cat])=><article className="article-row" key={title}><time>{date}</time><h2>{title}</h2><span>{cat}</span><b aria-hidden="true">→</b></article>)}</div></section><Footer/></main>}
